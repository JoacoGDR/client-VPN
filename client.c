...
int main(int argc, char* args[])
{
    ...
    char * assigned_ip = calloc(sizeof(char), 30);
    printf("Receiving assigned ip...\n");
    recv(sockfd, assigned_ip, 30, 0);
    printf("Assigned ip received: %s\n", assigned_ip);


    char* command = malloc(2000);
    printf("Running client interface...\n");
    sprintf(command, "./client_interface.sh %s %s %s %s", server_key, allowed_ips, server_endpoint, assigned_ip);
    system(command);
    free(command);
    // close the socket
    close(sockfd);
